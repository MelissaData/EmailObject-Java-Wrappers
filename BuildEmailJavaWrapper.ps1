...
$Files = @(
  ...
  [FileConfig]@{
    FileName       = "mdEmailJavaWrapper.cpp";
    ReleaseVersion = $RELEASE_VERSION;
    OS             = "ANY";
    Compiler       = "JAVA";
    Architecture   = "ANY";
...
function DownloadFiles() {
  ...
  foreach ($File in $Files) {
    ...
    if ($File.FileName -eq "mdEmail_JavaCode.zip") {
      if (!(Test-Path ("$CurrentPath\mdEmail_JavaCode.zip"))) {
        ...
        Expand-Archive -Path "$CurrentPath\mdEmail_JavaCode.zip" -DestinationPath $CurrentPath
        }
        else {
...
function CheckFiles() {
  ...
  if (!(Test-Path ("mdEmail.h"))) {
    Write-Host "mdEmail.h not found." 
    $FileMissing = $true
  }
  if (!(Test-Path ("mdEmail.lib"))) {
...
Write-Host "`n======================= WELCOME TO MELISSA EMAIL OBJECT JAVA WRAPPER =======================`n"
...
jar cvf mdEmail.jar com\melissadata\*.class > $null

if (Test-Path ("mdEmailJavaWrapper.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdEmailJavaWrapper.dll"

  Write-Host "`nmdEmailJavaWrapper.dll has been successfully generated at:`n$filePath"
}
else {
  Write-Host "`nError: was not able to create mdEmailJavaWrapper.dll"
}

if (Test-Path ("mdEmail.jar")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdEmail.jar"

  Write-Host "`nmdEmail.jar has been successfully generated at:`n$filePath"
}
else {
  Write-Host "`nError: was not able to create mdEmail.jar"
}

...
Config_FileName4="mdEmailJavaWrapper.cpp"
...
########################## Main ############################
printf "\n======================= WELCOME TO MELISSA EMAIL OBJECT JAVA WRAPPER =======================\n"
...
# Compile SO
gcc -fpic -c -I$ProjectPath -I$JavaIncludeDirectory1 -I$JavaIncludeDirectory2 mdEmailJavaWrapper.cpp
gcc -g -shared mdEmailJavaWrapper.o -o libmdEmailJavaWrapper.so -L$ProjectPath -lmdEmail -pthread 

rm mdEmailJavaWrapper.o

# Compile Jar
javac com/melissadata/*.java &> /dev/null

jar cvf mdEmail.jar com/melissadata/*.class &> /dev/null

if [ -f "libmdEmailJavaWrapper.so" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/libmdEmailJavaWrapper.so"

  printf "\nlibmdEmailJavaWrapper.so has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create libmdEmailJavaWrapper.so\n"
fi

if [ -f "mdEmail.jar" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/mdEmail.jar"

  printf "\nmdEmail.jar has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create mdEmail.jar\n"
fi
